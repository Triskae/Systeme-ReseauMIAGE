...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        switch (head->event) {
            case 'A': {
                prevTime = head->time;
                int ci = head->callerID;

                if (line == 0) {
                    insertEvent(&head, ci, 'D', prevTime + expntl(SERV_TIME));
                    line = 1;
                } else {
                    insertEvent(&bhead, ci, 'A', prevTime);
                }
                popEvent(&head);
    ...
    printf("*  Mean number in system    = %lf cust <--> THEORY:  %lf cust      \n", residenceTime / prevTime,
           lambda * (SERV_TIME + queueing_time));
